...
Une nouvelle fois, cela n'avait aucun sens pour moi. Dispatch s'appuyait sur Cowork pour exécuter les tâches et ce dernier était, en principe, exécuté dans un environnement sandboxé. Il n'aurait donc pas dû avoir accès à mon PC personnel. Par ailleurs, les informations que je demandais étaient disponibles sur l'API et Claude avait déjà réussi à y accéder sans rechigner. *Last but not least*, Claude ne m'avait ni prévenu ni demandé mon autorisation avant de lire le fichier de configuration SSH et de se connecter à mon Raspberry Pi. Bref, je m'étais retrouvé dans une situation où j'avais l'impression que Claude m'avait « échappé » des mains.
...
- Les vulnérabilités, les défauts de configuration et les dettes techniques font partie de nos SI. Comment contrôler un agent IA aussi capable que nos meilleurs experts techniques, mais qui, contrairement à eux, est infatigable ?
